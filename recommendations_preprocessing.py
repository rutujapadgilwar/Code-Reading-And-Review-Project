...
It includes methods for loading datasets, handling missing values, saving dataframes to pickle files,
and performing other preprocessing tasks.
...
class RecommendationsPreprocessing:
    ...
    df_books.drop(['Image-URL-S', 'Image-URL-L'], axis = 1, inplace = True) #dropping unrequired columns in books dataset
    ...
    df_books.loc[df_books['Year-Of-Publication'] == 'DK Publishing Inc',:] #checking data for 'DK Publishing Inc'
    ...
    df_users[['City', 'State', 'Country']] = df_users['Location'].str.split(', ', expand=True) # Splitting location into city, state and country
    ...
    df_popular_books = pd.merge(df_ratings_count, df_average_rating, on="Book-Title") # Merging total-ratings and average-ratings dataset

    df_top_books = df_popular_books[df_popular_books['Book-Rating']>=200].sort_values('Average-Rating',ascending=False) # Filter to consider total-ratings atleast more than 200
